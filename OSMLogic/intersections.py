...
def plotMap(route):
	...
	gmap.marker(coords[0][0], coords[1][0])
	gmap.plot(coords[0], coords[1],'cornflowerblue', edge_width=3)
	gmap.marker(coords[0][len(coords[0]) - 1], coords[1][len(coords[0]) - 1])
...
def getRouteCoords(route):
	...
	for x in range(0, len(route[0]) - 1):
		...
		for coord in coordsBetween:
			lats.append(coord[0])
			lons.append(coord[1])
	lats.append(route[0][len(route[0]) - 1])
	lons.append(route[1][len(route[0]) - 1])
